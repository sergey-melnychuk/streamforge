...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let rpc_server =
        RpcServer::new(Arc::clone(&transport)).with_membership(membership_for_rpc.clone());
    ...
    let mut discovery =
        GossipDiscovery::new(local_node_meta.clone(), config.clone()).with_transport(transport);
