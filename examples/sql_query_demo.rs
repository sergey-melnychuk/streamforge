...
use streamforge::distributed::{node::{NodeId, NodeMetadata}, ClusterMembership};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Contains {} sensor readings with timestamps\n", sensor_data.len());
    ...
    let query1 = SqlParser::parse("SELECT sensor_id, location, temperature FROM events WHERE temperature > 25.0")?;
    let executor1 = QueryExecutor::new(query1);
    
    let events1: Vec<Event> = stream1.collect().await?;
    let filtered: Vec<Event> = events1.iter()
        .filter(|e| executor1.matches_filter(e))
        .map(|e| executor1.project(e))
        .collect();
    
    println!("   Query: SELECT sensor_id, location, temperature FROM events WHERE temperature > 25.0");
    println!("   Results: {} events", filtered.len());
    for event in &filtered[..5.min(filtered.len())] {
        if let EventValue::Json(json) = &event.value {
            println!("     - Sensor: {}, Location: {}, Temp: {:.1}°C", 
                json.get("sensor_id").and_then(|v| v.as_str()).unwrap_or("N/A"),
                json.get("location").and_then(|v| v.as_str()).unwrap_or("N/A"),
                json.get("temperature").and_then(|v| v.as_f64()).unwrap_or(0.0));
        }
    }
    if filtered.len() > 5 {
    ...
    for result in &results3 {
        if let EventValue::Json(json) = &result.value {
            println!("     - Sensor: {}, Avg Temp: {:.2}°C, Readings: {}",
                json.get("sensor_id").and_then(|v| v.as_str()).unwrap_or("N/A"),
                json.get("avg_temp").and_then(|v| v.as_f64()).unwrap_or(0.0),
                json.get("reading_count").and_then(|v| v.as_i64()).unwrap_or(0));
        }
    }
    println!();
    ...
    let window_events: Vec<Event> = sensor_data.iter()
    ...
    let query5 = SqlParser::parse(
        "SELECT COUNT(*) AS event_count FROM events WINDOW TUMBLING 60 SECONDS"
    )?;
    ...
    for result in &results5[..5.min(results5.len())] {
        if let EventValue::Json(json) = &result.value {
            let window_start = json.get("window_start").and_then(|v| v.as_i64()).unwrap_or(0);
            let window_end = json.get("window_end").and_then(|v| v.as_i64()).unwrap_or(0);
            let count = json.get("event_count").and_then(|v| v.as_i64()).unwrap_or(0);
            
            println!("     - Window [{:.1}s - {:.1}s]: {} events",
    ...
    let window_events2: Vec<Event> = sensor_data.iter()
        .map(|(ts, sensor_id, temp, humidity, location)| {
            let json_val = json!({
                "sensor_id": sensor_id,
                "temperature": temp,
    ...
    let query6 = SqlParser::parse(
        "SELECT COUNT(*) AS event_count FROM events WINDOW SLIDING 60 SECONDS"
    )?;
    ...
    for (idx, result) in results7.iter().enumerate() {
        if let EventValue::Json(json) = &result.value {
            println!("     {}. Location: {}, Avg Temp: {:.2}°C, Readings: {}",
                idx + 1,
                json.get("location").and_then(|v| v.as_str()).unwrap_or("N/A"),
                json.get("avg_temp").and_then(|v| v.as_f64()).unwrap_or(0.0),
    ...
    let transport = Transport::bind(node1.address).await
    ...
    for partition in 0..12 {
        if let Some(node_id) = distributed_ctx.get_node_for_partition(partition).await {
            shuffle_manager.update_partition_mapping(partition, node_id).await;
        }
    }
    println!("  ✓ Partition mappings updated\n");
    
    // Create events with different sensor IDs to demonstrate partitioning
    let distributed_events: Vec<Event> = sensor_data.iter()
        .map(|(ts, sensor_id, temp, humidity, location)| {
            let json_val = json!({
                "sensor_id": sensor_id,
                "temperature": temp,
    ...
    for event in &distributed_events[..5.min(distributed_events.len())] {
        if let EventValue::Json(json) = &event.value {
            if let Some(sensor_id) = json.get("sensor_id").and_then(|v| v.as_str()) {
                ...
                if let Some(node_id) = distributed_ctx.get_node_for_partition(partition).await {
                    println!("     - Sensor '{}' → Partition {} → Node {}", 
                        sensor_id, partition, node_id);
                } else {
                    println!("     - Sensor '{}' → Partition {} → (unassigned)", 
                        sensor_id, partition);
                }
            }
        }
    ...
    let results10 = executor10.execute_with_aggregations(distributed_events.clone()).await
