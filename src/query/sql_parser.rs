...
use crate::query::ast::{JoinType, JoinCondition, JoinTable, OrderByClause, OrderByField, SortDirection, WhereClause, *};
use thiserror::Error;
...
impl SqlParser {
    ...
    /// Parse SELECT clause
    fn parse_select(tokens: &[String], start: usize) -> Result<(SelectClause, usize), SqlParseError> {
        ...
        while pos < tokens.len() && tokens[pos].to_uppercase() != "FROM" {
            ...
            if matches!(upper.as_str(), "COUNT" | "SUM" | "AVG" | "MIN" | "MAX" | "MEDIAN") {
                ...
                if pos < tokens.len() && tokens[pos].to_uppercase() == "AS" {
                    ...
                    pos += 1;
                    fields.push(SelectField::Aliased { field: field_name, alias });
                } else {
    ...
    fn parse_aggregation(tokens: &[String], start: usize) -> Result<(Aggregation, usize), SqlParseError> {
        let func_name = tokens[start].to_uppercase();
        let mut pos = start + 1;
        
        // Skip opening parenthesis
        if pos >= tokens.len() || tokens[pos] != "(" {
            return Err(SqlParseError::Syntax("Expected ( after aggregation function".to_string()));
        }
        pos += 1;
        ...
        if pos >= tokens.len() || tokens[pos] != ")" {
            return Err(SqlParseError::Syntax("Expected ) after aggregation".to_string()));
        }
        pos += 1;
        ...
        Ok((Aggregation { function: func_name, field, alias }, pos))
    }
    ...
    fn parse_from(tokens: &[String], start: usize) -> Result<(FromClause, usize), SqlParseError> {
        ...
        let mut pos = start + 1;
        if pos >= tokens.len() {
            return Err(SqlParseError::Syntax("Expected stream name after FROM".to_string()));
        }
        ...
        while pos < tokens.len() {
            ...
            if upper == "JOIN" || upper == "INNER" || upper == "LEFT" || upper == "RIGHT" 
                || (upper == "FULL" && pos + 1 < tokens.len() && tokens[pos + 1].to_uppercase() == "OUTER") {
                
                // Parse join type
                let join_type = if upper == "INNER" {
                    pos += 1;
                    if pos >= tokens.len() || tokens[pos].to_uppercase() != "JOIN" {
                        return Err(SqlParseError::Syntax("Expected JOIN after INNER".to_string()));
                    }
                    pos += 1;
                    JoinType::Inner
                } else if upper == "LEFT" {
                    pos += 1;
                    if pos >= tokens.len() || tokens[pos].to_uppercase() != "JOIN" {
                        return Err(SqlParseError::Syntax("Expected JOIN after LEFT".to_string()));
                    }
                    pos += 1;
                    JoinType::Left
                } else if upper == "RIGHT" {
                    pos += 1;
                    if pos >= tokens.len() || tokens[pos].to_uppercase() != "JOIN" {
                        return Err(SqlParseError::Syntax("Expected JOIN after RIGHT".to_string()));
                    }
                    pos += 1;
                    JoinType::Right
                } else if upper == "FULL" {
                    ...
                    if pos >= tokens.len() || tokens[pos].to_uppercase() != "OUTER" {
                        return Err(SqlParseError::Syntax("Expected OUTER after FULL".to_string()));
                    }
                    pos += 1;
                    if pos >= tokens.len() || tokens[pos].to_uppercase() != "JOIN" {
                        return Err(SqlParseError::Syntax("Expected JOIN after FULL OUTER".to_string()));
                    }
                    pos += 1;
                    JoinType::Outer
                } else {
                ...
                if pos >= tokens.len() || tokens[pos].to_uppercase() != "ON" {
                    return Err(SqlParseError::Syntax("Expected ON clause after JOIN".to_string()));
                }
                pos += 1;
                ...
                if pos >= tokens.len() {
                    return Err(SqlParseError::Syntax("Expected operator in JOIN condition".to_string()));
                }
                ...
                let op = match tokens[pos].as_str() {
                    ...
                    ">" => BinaryOperator::Gt,
                    ">=" => BinaryOperator::Ge,
                    _ => return Err(SqlParseError::Syntax(format!("Unsupported JOIN operator: {}", tokens[pos]))),
                };
                pos += 1;
                
                let (right_field, pos_after_right_field) = Self::parse_qualified_field(tokens, pos)?;
                ...
                let join_table = match right_table {
                    ...
                    _ => return Err(SqlParseError::Syntax("Right side of JOIN must be a single table".to_string())),
                };
    ...
    fn parse_table_reference(tokens: &[String], start: usize) -> Result<(FromClause, usize), SqlParseError> {
        if start >= tokens.len() {
        ...
        // Check for alias
        let alias = if pos < tokens.len() && tokens[pos].to_uppercase() != "JOIN"
            && tokens[pos].to_uppercase() != "INNER" && tokens[pos].to_uppercase() != "LEFT"
            && tokens[pos].to_uppercase() != "RIGHT" && tokens[pos].to_uppercase() != "FULL"
            && tokens[pos].to_uppercase() != "ON" && tokens[pos].to_uppercase() != "WHERE"
            && tokens[pos].to_uppercase() != "GROUP" && tokens[pos].to_uppercase() != "WINDOW"
            && tokens[pos].to_uppercase() != "ORDER" && tokens[pos].to_uppercase() != "LIMIT"
    ...
    fn parse_qualified_field(tokens: &[String], start: usize) -> Result<(String, usize), SqlParseError> {
        if start >= tokens.len() {
        ...
        if pos < tokens.len() && tokens[pos] == "." {
            pos += 1;
            if pos >= tokens.len() {
                return Err(SqlParseError::Syntax("Expected field name after .".to_string()));
            }
    ...
    /// Parse WHERE clause
    fn parse_where(tokens: &[String], start: usize) -> Result<(Option<WhereClause>, usize), SqlParseError> {
    ...
    fn parse_expression(tokens: &[String], start: usize) -> Result<(Expression, usize), SqlParseError> {
        if start >= tokens.len() {
    ...
    fn parse_expression_term(tokens: &[String], start: usize) -> Result<(Expression, usize), SqlParseError> {
        if start >= tokens.len() {
            return Err(SqlParseError::Syntax("Expected expression term".to_string()));
        }
    ...
    fn parse_group_by(tokens: &[String], start: usize) -> Result<(Option<Vec<String>>, usize), SqlParseError> {
        ...
        while pos < tokens.len() {
            ...
            if token_upper == "WINDOW" || token_upper == "HAVING" || token_upper == "ORDER" || token_upper == "LIMIT" {
                break;
            }
    ...
    /// Parse HAVING clause
    fn parse_having(tokens: &[String], start: usize) -> Result<(Option<WhereClause>, usize), SqlParseError> {
    ...
    fn parse_order_by(tokens: &[String], start: usize) -> Result<(Option<OrderByClause>, usize), SqlParseError> {
    ...
    /// Parse LIMIT clause
    fn parse_limit(tokens: &[String], start: usize) -> Result<(Option<usize>, usize), SqlParseError> {
        ...
        let mut pos = start + 1;
        if pos >= tokens.len() {
            return Err(SqlParseError::Syntax("Expected number after LIMIT".to_string()));
        }
        
        let limit = tokens[pos].parse::<usize>()
    ...
    /// Parse OFFSET clause
    fn parse_offset(tokens: &[String], start: usize) -> Result<(Option<usize>, usize), SqlParseError> {
        ...
        let pos = start + 1;
        if pos >= tokens.len() {
            return Err(SqlParseError::Syntax("Expected number after OFFSET".to_string()));
        }
        
        let offset = tokens[pos].parse::<usize>()
    ...
    fn parse_window(tokens: &[String], start: usize) -> Result<(Option<WindowSpec>, usize), SqlParseError> {
        ...
        let mut pos = start + 1;
        if pos >= tokens.len() {
            return Err(SqlParseError::Syntax("Expected window type after WINDOW".to_string()));
        }
        ...
        let window_type = match window_type_str.as_str() {
            ...
            _ => return Err(SqlParseError::Syntax(format!("Unknown window type: {}", window_type_str))),
        };
...
mod tests {
    ...
    fn test_parse_select_with_group_by() {
        let query = SqlParser::parse("SELECT sensor_id, AVG(temperature) FROM events GROUP BY sensor_id").unwrap();
    ...
    fn test_parse_select_with_window() {
        ...
        assert!(matches!(query.window.unwrap().window_type, WindowType::Tumbling));
    }
    ...
    fn test_parse_select_with_order_by() {
        ...
        assert_eq!(query.order_by.as_ref().unwrap().fields[0].direction, SortDirection::Desc);
    }
    ...
    fn test_parse_join_inner() {
        ...
        match &query.from {
            FromClause::Join { join_type, right, .. } => {
